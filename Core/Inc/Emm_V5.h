#ifndef __EMM_V5_H
#define __EMM_V5_H

#include "can.h"
#include "stdbool.h"

/**********************************************************
***	Emm_V5.0闭环步进电机驱动
**********************************************************/

#define		ABS(x)		((x) > 0 ? (x) : -(x))

/* 四个底盘电机的 CAN 地址 (与 2026-5校赛 项目 motor.h 一致) */
#define MOTOR_FL    0x01  /* 左前轮 */
#define MOTOR_FR    0x02  /* 右前轮 */
#define MOTOR_RL    0x03  /* 左后轮 */
#define MOTOR_RR    0x04  /* 右后轮 */

typedef enum {
	S_VER   = 0,			/* 读取固件版本和对应硬件版本 */
	S_RL    = 1,			/* 读取读取线性位移误差 */
	S_PID   = 2,			/* 读取PID参数 */
	S_VBUS  = 3,			/* 读取总线电压 */
	S_CPHA  = 5,			/* 读取相电流 */
	S_ENCL  = 7,			/* 读取电机线性位移校准的脉冲数 */
	S_TPOS  = 8,			/* 读取电机目标位置角度 */
	S_VEL   = 9,			/* 读取电机实时转速 */
	S_CPOS  = 10,			/* 读取电机实时位置角度 */
	S_PERR  = 11,			/* 读取电机位置误差角度 */
	S_FLAG  = 13,			/* 读取使能/到位/回零状态标志位 */
	S_Conf  = 14,			/* 读取电机参数 */
	S_State = 15,			/* 读取系统状态参数 */
	S_ORG   = 16,           /* 读取回零/回零失败状态标志位 */
}SysParams_t;


/**********************************************************
*** 注意：每个函数的参数的具体说明请看对应函数注释
**********************************************************/
void Emm_V5_Reset_CurPos_To_Zero(uint8_t addr); // 位置清零
void Emm_V5_Reset_Clog_Pro(uint8_t addr); // 清除堵转保护
void Emm_V5_Read_Sys_Params(uint8_t addr, SysParams_t s); // 读取参数
void Emm_V5_Modify_Ctrl_Mode(uint8_t addr, bool svF, uint8_t ctrl_mode); // 修改控制模式
void Emm_V5_En_Control(uint8_t addr, bool state, bool snF); // 使能控制
void Emm_V5_Vel_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, bool snF); // 速度模式控制
void Emm_V5_Pos_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint32_t clk, bool raF, bool snF); // 位置模式控制
void Emm_V5_Stop_Now(uint8_t addr, bool snF); // 立即停止
void Emm_V5_Synchronous_motion(uint8_t addr); // 触发同步运动
void Emm_V5_Origin_Set_O(uint8_t addr, bool svF); // 设置单圈回零零点
void Emm_V5_Origin_Modify_Params(uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF); // 修改回零参数
void Emm_V5_Origin_Trigger_Return(uint8_t addr, uint8_t o_mode, bool snF); // 触发回零
void Emm_V5_Origin_Interrupt(uint8_t addr); // 强制中断并退出回零

#endif
